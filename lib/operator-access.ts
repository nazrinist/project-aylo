...
export function getOperatorAccessSecret() {
  const token = configuredToken();
  return token.length >= MINIMUM_OPERATOR_TOKEN_LENGTH ? token : null;
}

export function isOperatorAccessConfigured() {
  return getOperatorAccessSecret() !== null;
}

export function verifyOperatorAuthorization(authorization: string | null) {
  const expected = getOperatorAccessSecret();
